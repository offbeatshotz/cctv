...
def capture_screenshot():
    ...
    filename = f"screenshot_{camera_id}_{timestamp}.png"
    ...
    try:
        ...
        return jsonify({'status': 'success', 'message': f'Screenshot from {camera_id} saved to {filepath}.'})
...
def upload_recording():
    ...
    camera_id = request.form.get('camera', 'unknown')
    
    if video_file:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"recording_{camera_id}_{timestamp}.webm"
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        
        try:
            video_file.save(filepath)
            print(f"Recording saved to {filepath}")
            return jsonify({'status': 'success', 'message': f'Recording from {camera_id} saved to {filepath}.'})
        except Exception as e:
